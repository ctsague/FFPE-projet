...
"Edit" => "편집",
"Suggestion" => "제안",
"Please answer the questions below" => "아래 질문에 내용을 채워주십시오",
...
"Remote service inaccessible (%0)" => "원격 서비스 접근 불가 (%0)",
...
"Category" => "범주",
"Allows the tracker item to be categorized in one or more categories under the specified main category" => "지정된 주 범주 하의 하나 혹은 여러 개의 범주가 트래커 항목에 의해 영향받는 것을 허용",
...
"Provides a one-line field for numeric input only. Prepended or appended values may be alphanumeric" => "수치 입력에 대하여서만 한 줄 필드를 제공합니다. 앞 혹은 뒤에 붙는 값은 알파벳과 숫자일 수 있습니다",
"Same Row" => "같은 행",
...
"Email" => "이메일",
"Allows an email address to be input with the option of making it active" => "이메일 주소 입력을 활성화할 수 있는 선택사항과 함께 허용",
"Link Type" => "링크 유형",
...
"Allows tags to be shown or added for tracker items" => "프리태그가 트래커 항목에 대하여 표시되거나 추가되는 것을 허용함",
...
"Assign" => "할당",
"Header" => "머리글",
"Displays a heading between fields to delimit a section and allow folding the fields" => "섹션을 구분하기 위하여 필드 간에 헤더 표시하고 필드 접기를 허용",
"Heading Level" => "헤더 레벨",
...
"Closed" => "닫힘",
"Icon" => "아이콘",
"Provides ability to select an image as an icon attached to the tracker item from file galleries" => "그림을 파일 갤러리에서 트래커 항목으로 첨부된 아이콘으로써 선택하는 것을 허용합니다",
"Default image" => "기본 그림",
...
"In Group" => "그룹 내부에",
"Indicates if the user associated with the item is member of a specified group" => "항목과 연관된 사용자가 지정된 그룹의 멤버인지를 나타냄",
"Group Name" => "그룹 이름",
...
"Provides a jQuery UI date picker to select a date and optionally time" => "날자와 선택적으로 시간으로 선택하기 위하여 jQuery-UI 날짜 선택기를 제공합니다",
"Default value" => "기본 값",
...
"Your rating" => "귀하의 등급",
"Relations" => "관계",
"Allows arbitrary relations to be created between the trackers and other objects in the system" => "트래커들과 시스템 내부의 다른 개체들 사이에 임의의 관계를 만드는 것을 허용",
"Relation" => "관계",
...
"The file has been locked by %s" => "파일은 %s 님이 잠궜습니다",
...
"Style" => "스타일",
"CSS style for positioning the module, etc" => "모듈을 위치조정에 대한 CSS 스타일링",
"Class" => "클래스",
...
"Saves your edits as you write, enabling work to be recovered after an \"interruption\". Also enables a \"live\" preview and is required for WYSIWYG plugin processing." => "진행 중 편집내용을 저장하여 \"장애\"로부터 작업내용을 복구할 수 있도록 합니다. \"실시간\" 미리보기를 활성화하며 wysiwyg 플러그인 처리에 요구됩니다.",
...
"Sets the number of comments per page (default = 10)" => "페이지 당 댓글의 수를 설정 (기본 = 10)",
"Default Ordering" => "기본 순서",
"sSets the default ordering filter for comments (default = points_desc)" => "댓글에 대하여 기본 정렬 필터를 설정함 (기본 = points_desc)",
...
"Sets the maximum width of article image" => "기사 그림에 대하여 최대 넓이를 설정함",
"Number of pixels, \"0\" for no maximum" => "픽셀수, 최대 없음은 0",
"Default maximum height for custom article images" => "사용자 지정 기사 그림에 대한 기본 최대 높이",
"Sets the maximum height of article images" => "기사 그림에 대하여 최대 높이를 설정함",
...
"Number of pixels (\"0\" to default to the view mode maximum)" => "픽셀수, 보기 모드 최대로 대체하려면 0",
...
"Sets the default maximum height of custom article images in list mode (on View Articles page)" => "목록 모드에서 사용자 지정 기사 그림 기본 최대 높이를 설정 (기사 보기에서)",
...
"Enables adding of copyright information for articles" => "기사에 대하여 저작권 정보를 추가하는 것을 허용",
...
"Allow users to share their access rights for the current page with a friend when sending the link by email. The lifespan of the link is defined by the site." => "사용자로 하여금 이메일로 링크를 보냈을 때 현재 페이지의 접근 권한을 공유하는 것을 허용. 링크의 수명은 사이트에 의하여 정의됨.",
...
"Create user if not already a registered user" => "티키에 없다면 사용자 만들기",
...
"2 minutes" => "2분",
"1 hour" => "1 시간",
"Categories used in templates (.tpl files)" => "범주가 템플릿 (TPL) 에서 사용됨",
"Enables showing alternate content, depending on the category of the current object" => "현재 개체의 범주에 따라 대체 컨텐츠 표시를 허용",
"Category jail" => "범주 감옥",
...
"Maximum number of \"more like this\" objects; otherwise, use the default maximum records" => "조금 더 이와 같은 개체의 최대, 그 외의 경우, 기본 최대 기록을 사용",
...
"Allow users with permission (tiki_p_vote_comments) to vote on comments." => "tiki_p_vote_comments 권한을 보유한 사용자에게 댓글에 대한 투표를 허용",
...
"weekly" => "주간",
"This feature is experimental and currently not in use. Stay tuned for updated information. Click \"Send Info\" to connect" => "실험적. 현재 사용되지 않음. 연결하려면 \"정보 전송\"을 클릭하십시오",
...
"Number of days to wait before re-validating the user's email address" => "사용자의 이메일을 재 유효화하기 전에 대기할 날의 수",
"days" => "일",
...
"Level of errors to be reported." => "어떠한 오류가 보고되어야 하는가에 대한 단계",
...
"Default order of listed comments" => "나열된 댓글에 대한 기본 정렬",
"Question and Answer prefix on Answers" => "답변들에 질문과 답변 접두어",
"Question and Answer prefix" => "질문과 대답 접두어",
"Q and A" => "질문과 답",
"Question ID" => "질문 ID",
"Blog:" => "블로그:",
"Limit categorization to within the subtree of" => "범주화를 다음의 하위트리로 강제 적용 및 제한",
...
"Collaboratively authored documents with edit history." => "변경 기록이 포함된 협업식으로 작성된 문서들",
"Blog" => "블로그",
"Online diaries or journals." => "온라인 일기 혹은 저널.",
"Collections of images for viewing or downloading (photo albums)" => "보기 및 다운로드를 할 수 있는 그래픽 이미지의 모음 (포토 앨범)",
"File galleries can be used instead." => "파일 갤러리를 대신 사용할 수 있음",
...
"Add to each page an \"Email this page\" link" => "\"이 페이지를 이메일로 보내기\" 링크를 모든 페이지에 추가",
"Share" => "공유",
...
"Allow logged-in users to post commments anonymously" => "로그인 사용자에게 댓글 게시 시 익명으로 가장 허용",
...
"CSS Menus" => "CSS 메뉴",
"CSS (Superfish) Menus" => "CSS 메뉴 (서커피시)",
...
"Wiki-page-based slideshow" => "웹 기반 슬라이드쇼",
"Slideshow (jQuery.s5) PDF Export" => "슬라이드쇼 (jQuery.s5) PDF 내보내기",
"Wiki-page-based slideshow to pdf export" => "웹 기반 슬라이드쇼를 pdf 로 내보내기",
"Invoice" => "청구서",
...
"Tiki will check for updates when the main Administration page is accessed" => "귀하께서 메인 관리 페이지를 접속하면 업데이트를 확인할 것입니다",
"Require confirmation of an action if a possible CSRF is detected" => "가능한 CSRF 감지된 경우 요청 확인",
...
"Allow flagging changes as urgent, so translations are marked with a notice visible to all users" => "변경내역을 긴급으로 표기하는 것 허용, 이는 번역이 모든 사용자에게 보이는 알림으로 표기되도록 함",
...
"Structures search for equivalent pages in other languages. May cause performance problems with larger structures" => "구조체가 다른 언어로 된 동등한 페이지를 조회 함. 대형 구조체들에서는 성능 문제가 발생할 수 있습니다",
...
"Help System" => "도움말 시스템",
"Activates links to Tiki documentation" => "티키의 도움말 시스템 사용",
"Smileys" => "이모티콘",
...
"Hide buttons in Draw, HTML id, comma-separated. Example: \"tool_select,tool_fhpath,tools_line_show,tools_rect_show,tools_ellipse_show,tool_path,tools_shapelib_show,tool_text,tool_image,tool_zoom,tool_eyedropper\"" => "티키 그리기에서 발견된 버튼 숨기기, HTML id, 쉼표로 구분됨: \"tool_select,tool_fhpath,tools_line_show,tools_rect_show,tools_ellipse_show,tool_path,tools_shapelib_show,tool_text,tool_image,tool_zoom,tool_eyedropper\"",
...
"Use File Galleries to store images" => "그림 저장에 파일 갤러리 사용",
...
"When a menu isn't set as a Bootstrap or CSS menu, use folder icons for menu section items (rather than plus/minus signs); defaults to \"y\"" => "메뉴가 폴더로써 표시되어야 하는가, 기본은 \"y\"",
...
"Submissions" => "투고",
"Submissions are submitted but need to be approved before they are published" => "제출이 전달되었습니다만, 라이브로 가기전에 승인 받아야만 합니다",
"Rankings" => "순위",
...
"Show the list of last-changed items" => "최종 변경된 항목들의 목록 표시",
"Orphan pages" => "고아 페이지",
...
"Enable ranking of blog posts" => "블로그 항목에 대하여 순위 사용",
...
"Direct import of local files into a file gallery. This method can handle large files. The user can upload files via FTP to the target folder" => "파일 갤러리로 로컬 파일들을 직접 가져오기. 대형 파일 처리 가능. 사용자는 대상 폴더로 FTP 를 통하여 파일 업로드 할 수 있습니다",
"Use batch uploading to transfer large files to file galleries" => "큰 파일들을 파일 갤러리로 통합을 위해 일괄 업로딩 사용",
...
"Display a site logo image and/or title" => "사이트 로고와 제목을 사용",
...
"Provide a warning to a user who attempts to edit a page that it is currently being edited by another user" => "티키는 다른 사용자가 현재 편집 중인 페이지를 편집 시도하는 사용자에게 경고할 것입니다",
"Undo" => "복구",
"Footnotes" => "각주",
"Enable creating private notes for a page that are visible only to the author" => "저자에게만 보이는 페이지에 대한 개인 메모 만들기",
"Allow HTML" => "HTML사용 허가",
"Log bytes changes (+/-) in action logs" => "작업 로그에서 바이트 변경을 기록 (+/-)",
"May impact performance" => "성능에 영향을 미칠 수 있습니다",
"Sandbox" => "모래상자",
"A special wiki page for testing that can edited but not permanently saved" => "시험을 위한 특별 위키 페이지. 사용자는 편집할 수 있지만 모래상자를 저장할 수 없음",
...
"permit exporting wiki pages" => "위키 내보내기 헝요",
"WikiWords" => "위키단어",
...
"Dumps" => "덤프",
"Enable exporting the site as a downloadable archive" => "사이트를 다운로드 가능한 기록 보관 파일로 덤핑을 허용",
"Wiki:" => "위키:",
...
"If an error message indicates a mandatory category is required when editing an image (in an image gallery), set this option to \"None\"" => "만약 (그림 갤러리 내부의) 그림을 편집 시, 필수 범주가 요구됨을 나타내는 오류 메시지를 받게된다면, 이 선택사항이 그 이유입니다. None (없음) 으로 설정하십시오",
"Show/hide icon before headings" => "제목 전에 표시/숨김",
"Display before headings an icon to show or hide the content that follows the headings" => "제목 앞에 표시되는 표시/숨김 아이콘 표시",
...
"Confirm that the Admin account has a valid email address or you will not be able to log in" => "Admin 계정이 유효한 이메일 주소를 보유하고 있다는 것을 확인하십시오 아니면 로그인이 허용되지 않습니다",
...
"Fixed width" => "수정된 넓이",
"Sets the width of the site contents display" => "사이트 넓이를 제한함",
...
"Invite users" => "사용자 초대",
"Allow users to invite new users by mail to register at this site" => "사용자들이 새 사용자에게 이 티키로 등록하도록 이메일을 보내 초대하는 것을 허용함",
"Alternate registration page" => "대체 등록 페이지",
"Redirects requests for tiki-register.php to an alternate registration page" => "tiki-register.php 로의 모든 접근을 등록을 위한 대체 페이지로 리디렉션하게 합니다",
...
"Enables syntax highlighting provided by CodeMirror" => "코드미러를 사용하여 문법 강조 활성화",
...
"RSS feeds for individual image galleries" => "개별 그림 갤러리에 대한 RSS 피드",
...
"Latest images uploaded to the image galleries" => "이미지 갤러리로 업로드된 마지막 그림들",
"Individual image galleries RSS Description" => "개별 그림 갤러리 RSS 설명",
"Description to be published as part of the RSS feeds for individual image galleries" => "개별 그림 갤러리에 대한 RSS 피드의 일부로써 게재될 설명",
"Latest images uploaded to this image gallery." => "이 이미지 갤러리로 업로드된 마지막 그림들.",
...
"Display the .zip option in gallery" => "갤러리 내부의 zip 선택사항 표기",
...
"Specify a directory on your server, for example: /var/www/  It's recommended that this directory not be web accessible. PHP must be able to read/write to the directory" => "서버 상에 디렉터리를 설정하십시오. 예.: /var/www/  이 디렉터리는 웹으로 접근이 가능하지 않도록 하는 것을 권장합니다. PHP 는 이 디렉터리에 반드시 쓰기/읽기가 가능해야만 합니다",
...
"Do not start messages titles with 'Re: '" => "제목을 'Re: ' 로 시작하지 않습니다",
...
"Include \"Generate password\" option in registration form" => "\"비밀번호 생성\" 선택사항을 등록 서식에 포함",
...
"LVisible label in the browser's title bar on all pages. Also appears in search engine results" => "모든 페이지에서 브라우저의 제목 바에 보이는 레이블. 검색 엔진에서도 표시됨",
"Validate new user registrations by email" => "이메일로 새 사용자 등록 확인",
"UUpon registration, the new user will receive an email containing a new-account validation link." => "등록 시, 신규 사용자는 유효성을 확인하는 링크가 담긴 이메일을 받게 됩니다.",
...
"Each group can have its own theme" => "각가의 그룹은 자신만의 테마를 보유할 수 있습니다",
"Subtitle" => "부제",
...
"Display a tracker (form) for the user to complete as part of the registration process. This tracker will be used to store additional information about each user." => "등록 절차의 일부로써 사용자가 작성을 마쳐야 할 트래커 (서식)을 표기합니다. 이 트래커를 사용하여 각 사용자에 대하여 추가 정보를 저장하십시오.",
"Go to the \"Admin Groups\" page to select which tracker and fields to display" => "어떤 트래커와 항목을 표기할 것인지 선택하기 위하여 \"관리 그룹\" 페이지를 사용하십시오",
...
"Includes a custom block of CSS in the <html> <head> section of all pages" => "모든 페이지 내부에 CSS 인라인의 사용자 지정 블럭을 포함합니다",
...
"HTML DOM Elements to be fixed" => "수정될 HTMLDomElements",
...
"Constrains the width of the content area of the site (default: 1170px)" => "사이트 표시 넓이를 제한함 (기본: 990px)",
...
"Obscure the email address when using the email address as username if possible (coverage will not be complete)" => "유저명으로 이메일 사용 시 이메일을 모호하게 함 (보호는 완벽하지 않음)",
"This will attempt as much as possible to hide the email address, showing the real name or the truncated email address instead." => "이메일 대신 실명 혹은 축약된 이메일 주소를 표시하여, 이메일을 최대한 숨기려는 시도를 하게 됩니다.",
...
"Menu icons" => "메뉴 아이콘",
"Allows icons to be defined for menu entries" => "메뉴 항목에 대한 아이콘 정의 허용",
...
"Use the image title instead" => "그림 제목을 대신 사용",
...
"Use each page description as a meta tag for that page" => "개별 페이지 설명을 페이지에 대한 메타 태그로써 사용",
"Author" => "저자",
...
"Meta robots" => "메타 로봇",
"Specify how Web robots should index your site. Valid values include: INDEX or NOINDEX, and FOLLOW or NOFOLLOW" => "웹봇이 귀하의 사이트를 어떻게 색인해야하는지를 지정합니다. 유효한 값들은 다음을 포함합니다: INDEX 혹은 NOINDEX, 그리고 FOLLOW 혹은 NOFOLLOW",
"Revisit after" => "다음 이 후 재 방문",
"Specify how often (in days) Web robots should visit your site" => "귀하의 사이트를 웹봇이 얼마나 자주 (일 단위) 방문해야하는지를 지정합니다",
...
"Activate zone for modules such as site logo, log-in form, etc. (page header)" => "로고, 로그인 등의 (헤더)와 같은 모듈을 유지할 영역의 표시 여부",
"Topbar module zone" => "Topbar 모듈 영역",
"Activate zone for modules such as horizontal menu (navbar), search form, page-wide content, etc" => "메인 가로 메뉴, 검색 형식, 페이지 너비 컨텐츠, 기타 등등과 같은 모듈들의 영역의 표시 여부",
"topbar" => "TopBar",
"Page top module zone" => "페이지 상단 모듈 영역",
"Activate zone for modules such as breadcrumbs, banners, share icons, etc" => "공유와 같은 모듈들을 유지하기 위한 영역의 표시 여부",
"Bottom module zone" => "하단 모듈 영역",
"Activate zone for modules such as \"powered by\" and \"rss list\" (page footer)" => "\"powered by\" 및 \"rss list\" 와 같은 모듈을 유지할 영역 (바닥글)의 표시 여부",
"Page bottom module zone" => "페이지 하단 모듈 영역",
"Activate zone for modules at the bottom of the main column of each page" => "각 페이지의 하단에 모듈을 유지할 영역의 표시 여부",
"Module file" => "모듈 파일",
...
"Multi-domain" => "다중 도메인",
"Allows domain names to be mapped to perspectives and simulate multiple domains hosted with the same Tiki installation" => "도메인명을 퍼스펙티브로 매핑하고 동일한 인스턴스 상에 호스트된 멀티도메인을 시뮬레이트하는 것을 허용합니다",
...
"Enable IPN for automatic payment completion. When enabled, PayPal will ping back the site when a payment is confirmed. The payment will then be entered automatically. This may not be possible if the site is not on a public server" => "자동 지불 완료에 대한 IPN 활성화. 활성화되었을 때, 지불이 확인되었을 때 페이팔이 사이트로 다시 ping을 할 것임. 그 후 지불이 자동적으로 입력될 것임. 이는 서버가 공용서버 상에 있지 않으면 가능하지 않을 수 있음",
"Currency used when entering payments" => "지불내역을 입력할 때 사용되는 통화",
"Default acceptable payment delay" => "기본 수용가능한 지불 연기",
"Number of days before the payment requests becomes overdue. This can be changed per payment request" => "지불 요청이 기한이 지나기 전까지의 날짜 수. 지불 요청 당 변경 가능",
...
"Each registry in Cclite can have its own currency. Must be one per registry. (case sensitive)" => "Cclite 의 각각의 레지스트리는 자신만의 통화를 보유할 수 있습니다. 레지스트리당 하나여야만 합니다. (대소문자 구분)  ",
...
"Username in Cclite representing \"the management\". Defaults to \"manager\"" => "Cclite dptj \"관리\"를 대표하는 사용자명. 기본은 \"관리자\"임",
...
"To do" => "할 일",
"Wiki page containing instructions to send a manual payment such as a check" => "수표와 같이 수동 지불을 발송하기 위한 설명을 포함하는 위키 페이지",
"Wiki page" => "Wiki페이지",
...
"The user can only see his or her own outstanding payments" => "사용자는 자신의 미납 지불만 볼 수 있습니다",
...
"The user can only see his or her own past or cancelled payments" => "사용자는 자신의 과거 혹은 취소된 지불만 볼 수 있습니다",
...
"In the products tracker, the ID of the field to store the total inventory of product" => "제품의 전체 재고를 저장할 제품 트래커 내부의 필드 ID",
"In the products tracker, the ID of the field to store the total inventory of product less the amount on hold because they are currently in carts" => "현재 카트 안에 있기 때문에 대기 상태인 양 만큼을 제외한 제품의 전체 재고를 저장할 제품 트래커 내부의 필드 ID",
...
"Activate bundled products feature. This requires a products tracker to be set up properly, and the item ID must be the product code" => "내장된 제품 기능을 활성화, 제대로 설치되려면 제품 트래커가 필요함, 항목ID(itemId) 는 반드시 제품 코드이어야 함",
...
"In the products tracker, the field name for the Associated Event ID. This is needed for the Associated Events cart feature, and requires an Events tracker to be set up in which the item ID there is the event ID to associate with" => "관련 이벤트 ID의 제품 트래커 내부의 필드명, 관련 이벤트 카트 기능에 대하여 필요함, 이벤트 ID가 연계가 될 항목 ID가 있는 곳에 설정될 이벤트 트래커를 필요로함",
"Product Class ID Field Name" => "제품 클래스 ID 필드명",
"In the products tracker, the name of the field for the Product Class ID, needed for the Gift Certificates cart feature" => "제품 클래스 ID의 제품 트래커 내부의 필드명, 상품권 카트 기능에 대하여 필요함",
"Gift Certificate Template Field Name" => "선물 상품권 템플릿 필드 명",
"In the products tracker, the name of the field for the Gift Cert Template, needed for the Gift Certificates cart feature" => "상품권 템플릿의 제품 트래커 내부의 필드명, 상품권 카트 기능에 대하여 필요함",
"Record cart orders in trackers (registered users)" => "트래커에 카트 주문 기록 (등록된 사용자)",
"This feature requires two trackers to be created and configured, an Orders tracker and an Orders Item tracker. Profiles also need to be configured, to do the recording" => "이 기능은 2개의 트래커를 요구합니다, 그리고 Orders 트래커와 Orders Item 트래커 가 구성되어야 합니다. 기록을 하려면 프로필이 구성되어야 합니다",
"Orders Profile" => "주문 프로필",
"Wiki page containing the profile for creating orders (page name must not have spaces)" => "주문을 만들기 위한 프로필이 있는 위키 페이지 (페이지명에는 스페이스가 없어야 함)",
"Order Item Profile" => "주문 항목 프로필",
"Wiki page containing the profile for creating orders items (page name must not have spaces)" => "주문 항목 만들기 위한 프로필이 있는 위키 페이지 (페이지명에는 스페이스가 없어야 함)",
"Allow anonymous shopping and record the orders in trackers" => "익명 구매 허용 및 주문을 트래커에 기록",
"Allows shopping by anonymous users" => "익명 사용자로 쇼핑 허용",
"Anonymous Orders Profile" => "익명 주문 프로필",
"Wiki page containing the profile for creating orders for anonymous users (page name must not have spaces)" => "익명 사용자를 위한 주문을 만들기 위한 프로필이 있는 위키 페이지 (페이지명에는 스페이스가 없어야 함)",
...
"Allow association of product orders with events (or projects, etc.)" => "제품 주문을 이벤트 (혹은 프로젝트 및 기타)로 연결을 허용",
"Allow association of products with events (or projects, etc.)" => "제품을 이벤트 (혹은 프로젝트 및 기타)로의 연결을 허용",
...
"ID of the tracker that is the Gift Certificate tracker" => "선물상품권 트래커인 트래커의 트래커ID",
...
"On \"Permission denied\", display the log-in module (for anonymous users)" => "권한 거부 시, 로그인 모듀을 표시합니다 (익명사용자에 대하여)",
...
"Displays smiley faces for a simple ratings system with an easy-to-use user interface, similar to emoticons. Works with 0-11 only, depending on the configuration" => "단순 등급 시스템에 대하여 웃는 얼굴 표시. 0-11 사이에서만 작동하며 설정된 내용에 따름. 이모티콘처럼 사용 시 더욱 쉬운 사용자 인터페이스",
"Enabled" => "활성화됨",
...
"If enabled, only search content that is in the interface language, otherwise show the language menu" => "활성화된 경우, 인터페이스 언어로 된 컨텐츠만 검색, 그 외의 경우 언어 메뉴 표시",
...
"Close the site (except for those with access permission)" => "사이트 닫기 (권한 보유자들은 제외)",
...
"HTML \"alt\" tag description (for text browsers, screen readers, etc.)" => "Alt. 설명 (예: 텍스트 브라우저 용",
...
"Even when the user is not logged in to Facebook, Tiki can access it" => "사용자가 페이스북에 로그인 되지 않았을 경우에도, 티키는 접근 가능합니다.",
"Tiki can post to the Facebook Wall" => "티키가 담벼락에 게시 가능",
"Tiki may post status messages, notes, photos, and videos to the Facebook Wall" => "티키는 페이스북 담벼락에 상태 메시지, 메모, 사진, 및 동영상을 게시할 수 있습니다",
...
"Tiki can SMS via Facebook" => "티키는 페이스북을 통하여 SMS 할 수 있습니다",
...
"Maximum number of results to produce. Results beyond these will need a more refined query to be reached" => "노출할 결과의 최대양. 이를 초과하는 결과에 도달하려면 좀 더 정제된 질의가 필요합니다",
...
"Lucene maximum number of buffered documents" => "Lucene 최대 버퍼된 문서",
...
"Lucene maximum number of merge documents" => "Lucene 최대 병합 문서",
...
"Small values (for example, less than 10,000) are best for interactive indexing. Use 0 for the Lucene default, which is practically infinite" => "대화형 색인에 대하여서는 작은 값 (예, 10,000보다 작음)이 최고입니다. 루씬 기본에 대하여서는 0을 사용하십시오, 이는 실직적으로는 무제한입니다",
...
"Large values (greater than 10) are best for batch index creation, and smaller values (less than 10) for indices that are interactively maintained" => "큰 값들 (> 10) 이 일괄 색인 만들기 용으로 최고입니다, 그리고 적은 값들 (< 10) 은 대화형으로 유지되는 색인용입니다",
...
"If enabled, forum replies will be indexed together with the initial post as a single document instead of being indexed separately" => "활성화 될 경우, 게시판 답글들은 별도로 색인되는 대신, 루트 게시와 함께 하나의 문서로써 함께 색인될 것입니다",
...
"Parse the results. May impact performance" => "결과를 구문분석. 성능에 영향을 미칠 수 있음",
...
"Close the site when the server load is above the threshold (except for users with closed-site access permission)" => "서버 부하가 임계치를 넘었을 때 사이트 닫기 (권한 보유자들은 제외)",
"Use proxy" => "프록시 사용",
"Use context menus for actions (icons)" => "작업에 대한 상황적 메뉴 사용 (아이콘)",
"Use context menus for actions (text)" => "작업에 대한 상황적 메뉴 사용 (텍스트)",
"Show user's real name instead of username (log-in name), when possible" => "로그인 대신 사용자의 실명 표시 (가능할 경우)",
...
"Number of days to consider in displaying \"who viewed my items\"" => "누가 내것을 보았는가를 고려할 날의 수",
...
"Prevents out-of-memory and performance issues when the user list is very large, by using a jQuery autocomplete text input box" => "사용자 목록이 매우 큰 경우 jQuery 자동완성 텍스트 입력 상자를 사용하여 메모리 부족과 성능 문제를 예방",
"Show user's real name instead of log-in name in the autocomplete selector in trackers" => "트래커 기능 안의 자동완성 선택기에서 로그인 대신 사용자의 실명을 표시합니다",
"Use user's real name instead of log-in name in the autocomplete selector in trackers" => "트래커 기능에서 자동완료 선택기 내부에서 사용자의 실명을 로그인 대신 사용",
"Show user's real name instead of log-in name in the autocomplete selector in the messaging feature" => "메시지 기능에서 자동완료 선택기에서 사용자의 로그인 대신 실명을 표시",
"Use user's real name instead of log-in name in the autocomplete selector in the messaging feature" => "메시징 기능에서 자동완료 선택기 사용자의 실명을 로그인 대신 사용",
"User Favorites" => "사용자 즐겨찾기",
"Allows users to flag content as their favorite." => "사용자가 컨텐츠를 자신의 즐겨찾기로 지정하는 것을 허용용",
...
"User tracker's field ID whose value is used as output page name" => "사용자 트래커 필드 ID. 이 값은 출력 페이지명으로써 사용됨",
"Quota (MB)" => "할당 (MB)",
"Private" => "비공개",
...
"Allow viewing HTML emails?" => "HTML 메일 보기 허용?",
"Maximum size for each attachment" => "각 첨부파일에 대한 최대 크기",
"Unlimited" => "무제한",
"Last editor" => "최종 편집자",
"Wiki link format" => "위키 링크 서식식",
"Level of special characters acceptable in wiki links for page names. For example: ((Page &eacute;&agrave;&icirc;))" => "페이지명에 대한 위키 링크로서 특수 문자의 어떤 단계까지 허용되는지. 예: ((Page &eacute;&agrave;&icirc;))",
...
"Allows the style in which the author list is displayed to be modified on a per-page basis" => "사용자 목록이 페이지 당 기반으로 표시되는 스타일로 수정하는 것을 허용합니다",
...
"Maximum length (in characters) to display before truncating backlink page names. Use \"0\" for no truncating" => "백링크 페이지명에 대하여 표시/자르기할 최대 길이 (글자). 자르기 하지 않으려면 \"0\" 을 사용",
...
"Enables the sectioning of a wiki page's content into two or more paginated pages" => "위키페이지를 구분자를 사용하여 페이지 매김된 페이지로 분리하는 것을 허용",
"Wiki page separator" => "위키페이지 분리자",
"Separator to use in the content of a wiki page to divide the content into multiple pages" => "위키 페이지의 컨텐츠 내부에서 컨텐츠를 분할하기 위해서 사용될 분리 기호",
"Plugin %0" => "%0 플러그인",
...
"This allows a mixture of wiki syntax and HTML syntax. All wiki syntax is parsed" => "이는 위키와 HTML의 혼합을 허용합니다. 모든 위키 문법이 구문분석됩니다",
"Content is partially wiki parsed" => "내용이 부분적으로 위키 구문분석 됩니다",
"This also allows a mixture of wiki syntax and HTML syntax. Only some wiki syntax is parsed, such as plugins (not inline character styles, etc)" => "이는 위키와 HTML의 혼합을 허용합니다. 플러그인과 같은 일부 위키 문법만이 구분분석됩니다 (인라인 캐릭터 스타일 등은 아닙니다)",
...
"Allow keeping wiki syntax with the WYSIWYG editor. Sometimes referred to as a \"visual wiki\"" => "WYSIWYG 편집기로 위키 문법을 유지하는 것을 허용. 때로는 \"비주얼 위키\"로도 알려져 있음",
"Font names" => "폰트명",
...
"Mail Delivery" => "메일 발송",
"When set to Queue, messages will be stored in the database. Requires using the shell script ./sendmail.php to be run for actual delivery. Only works with SMTP mail" => "Queue로 설정될 때 메시지를 데이터베이스에 저장될 것입니다. 실제 전달을 위해서는 쉘스크립트가 실행되는 것이 요구됩니다. SMTP메일에서만 작옹합니다",
...
"Downloads" => "내려받기수",
"Wiki most-recent images" => "위키 마지막 이미지",
"Upload date" => "업로드일",
...
"Upload a new image to a gallery" => "새 그림을 갤러리로 업로드",
"View another user's gallery" => "다른 사용자의 갤러리 보기",
"View another user's images" => "다른 사용자의 그림 보기",
"Have your gallery viewed by another user" => "갤러리를 다른 사용자가 보게 하십시오",
"Have your image viewed" => "그림을 보게 하십시오",
...
"The active index could not be removed, probably due to a file permission issue" => "활성 색인을 삭제할 수 없음. 파일 권한 문제일 가능성 큼",
"The new index could not be made active, probably due to a file permission issue" => "새 색인을 활성으로 전송할 수 없음. 파일 권한 문제일 가능성 큼",
"blog post" => "블로그 게시물",
"forum post" => "게시판 게시물",
"tracker item" => "트래커 항목",
"sheet" => "시트",
...
"Can read a topic (applies only to individual topic permissions)" => "토픽 읽기 가능 (개별 토픽 권한에만 적용)",
...
"Can assign permissions to image galleries" => "그림 갤러리에 권한 할당 가능",
"Can upload .zip files of images" => "ZIP 파일로 그림 업로드 가능",
...
"Postal Code" => "우편번호",
"(DOT)" => "(점)",
"Is the mailbox not initialised?" => "메일상자 초기화되지 않음??",
"No subject" => "토픽 없음",
...
"Product Type" => "제품 유형",
"The product type that is being sold, which will affect fulfillment, for example, standard product, gift certificate, event ticket" => "판매되는 제품 유형, 이는 처리에 영향을 미치게 될 것입니다, 예, 표준 제품, 선물 상품권, 이벤트 티켓",
...
"The bundle the product belongs to. Can be used to limit how gift cards are used, will automatically add other products in the same class to the cart" => "제품이 속한 번들, 선물카드가 어떻게 사용되는지를 제한하기 위하여 사용될 수 있으며 , 카트에 동일한 클래스 내부에 다른 제품을 추가할 것입니다",
"Bundle Class" => "번들 클래스",
...
"Log in to edit Convene" => "소집을 편집하기 위하여 로그인",
"Add" => "추가",
"Cookie" => "쿠키",
...
"Originating email address for mails from this forum" => "이 게시판에서의 메일에 대한 원래 이메일 주소",
...
"Warning: changing the username could require the user to change his or her password (for user registered with an old Tiki&lt;=1.8)" => "경고: 사용자명을 변경하면 사용자가 비밀번호를 변경해야만 할 수도 있습니다 (구버전 티키 1.8으로 등록한 사용자에 해당)",
"Warning: it will create a problem with Intertiki slave sites that use this one as master" => "경고: 이 사이트를 마스터로 사용하는 슬레이브 인터티키 사이트에 문제가 발생할 것입니다",
...
"Input list of participants, separated by commas. Roles must be indicated by a prefix separated by a colon as in" => "쉼표로 구분된느 참여자 목록을 주십시오. 역할은 다음에서처럼 열에 의해 구분되는 접두어가 주어져야만 합니다",
...
"Add page name as an h1-size header at the beginning of the page content" => "페이지 컨텐츠 시작 부분에 헤더 ! 로써 페이지 이름 추가",
...
"Personal email accounts" => "개인 이메일 계정",
"Group email accounts" => "그룹 이메일 계정",
...
"The uploaded file is not recognized as a image" => "업로드된 파일은 그림으로써 인식되지 않았습니다",
...
"There was a problem deleting that email message" => "그 메일을 삭제하는데 문제가 있었습니다",
"Mail ID" => "메일 ID",
"There was a problem while trying to delete these email messages" => "이 메일들을 삭제하려할 때 문제가 있었습니다",
"TThere was a problem while trying to delete this email message" => "그 메일을 삭제하려 시도했을 때 문제가 있었습니다",
"Are you sure you want to delete this account" => "이 계정을 삭제하시겠습니까?",
"Putting back" => "돌려놓기",
"Checking" => "체크 중",
"Aborted" => "중단됨",
...
"The Tidy extension is not present" => "Tidy 확장기능 없음",
...
"There was an error deleting the file" => "파일 삭제 오류",
"TikiTest List" => "티키테스트 목록",
"TikiTest Record" => "티키테스트 기록",
"The pecl HTTP extension or the Curl extension is needed to replay the TikiTest" => "티키테스트를 재생하려면 확장기능 pec HTTL 혹은 Curl 중 하나가 필요합니다",
