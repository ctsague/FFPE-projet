{* $Id: mod-translation.tpl 57623 2016-02-28 07:35:29Z gezzzan $ *}
				...
				<div>
							...
							{foreach item=better from=$mod_translation_better_known}
								<li>
									{if $from_edit_page eq 'y'}
										<a title=":{tr}update from it{/tr}" href="tiki-editpage.php?page={$page|escape:'url'}&amp;source_page={$better.page|escape:'url'}&amp;oldver={$better.last_update|escape:'url'}&amp;newver={$better.current_version|escape:'url'}&amp;diff_style={$default_diff_style}" class="tips">{icon name='refresh' istyle="vertical-align:middle"} {$better.lang|langname}</a> ({$better.page|escape})
									{else}
										{if $tiki_p_edit eq 'y'}
											<a title=":{tr}update from it{/tr}" href="tiki-editpage.php?page={$page|escape:'url'}&amp;source_page={$better.page|escape:'url'}&amp;oldver={$better.last_update|escape:'url'}&amp;newver={$better.current_version|escape:'url'}&amp;diff_style={$default_diff_style}" class="tips">{icon name='refresh' istyle="vertical-align:middle"}</a>
									{/if}
					...
					{if $mod_translation_better_other}
						{if $prefs.change_language eq 'y'}
							<a href="javascript:void(0)" onclick="intro=document.getElementById('mod-translation-better-intro');if(intro)intro.style.display='none';document.getElementById('mod-translation-better-ul').style.display='block';this.style.display='none'" class="linkmenu more">{icon name='menu-extra' istyle="vertical-align:middle;border:0"} {tr}More...{/tr}</a>
						{/if}
								...
								<li>
									...
									{else}
										{if $tiki_p_edit eq 'y'}
											<a title=":{tr}update from it{/tr}" href="tiki-editpage.php?page={$page|escape:'url'}&amp;source_page={$better.page|escape:'url'}&amp;oldver={$better.last_update|escape:'url'}&amp;newver={$better.current_version|escape:'url'}&amp;diff_style={$default_diff_style}" class="tips">{icon name='refresh' istyle="vertical-align:middle"}</a>
										{/if}
										<a href="tiki-index.php?page={$better.page|escape:'url'}&amp;no_bl=y" class="tips" title=":{tr}View{/tr}">{icon name='view' istyle="vertical-align:middle"}</a>
			...
			{if $mod_translation_worst_known or $mod_translation_worst_other}
				<div>
					...
					{if $mod_translation_worst_known}
						<ul>
							{foreach item=worst from=$mod_translation_worst_known}
								<li>
									{if $tiki_p_edit eq 'y'}
										<a href="tiki-editpage.php?page={$worst.page|escape:'url'}&amp;source_page={$page|escape:'url'}&amp;oldver={$worst.last_update|escape:'url'}&amp;newver={$pageVersion|escape:'url'}&amp;diff_style={$default_diff_style}" class="tips" title=":{tr}update it{/tr}">{icon name='refresh' istyle="vertical-align:middle"}</a>
									{/if}
					...
					{if $mod_translation_worst_other}
						{if $prefs.change_language eq 'y'}
							<a href="javascript:void(0)" onclick="intro=document.getElementById('mod-translation-worst-intro');if(intro)intro.style.display='none';document.getElementById('mod-translation-worst-ul').style.display='block';this.style.display='none'" class="linkmenu more">{icon name='menu-extra' istyle="vertical-align:middle;border:0"}{tr}More...{/tr}</a>
						{/if}
