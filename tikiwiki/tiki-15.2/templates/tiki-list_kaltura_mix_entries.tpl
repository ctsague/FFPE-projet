{* $Id: tiki-list_kaltura_mix_entries.tpl 57779 2016-03-05 17:09:00Z jonnybradley $ *}
{if $cant > 0}
	<div class="table-responsive">
		<table class="table" id="selectable">
			<tr>
				...
				<th width="20"><a href='#'{popup fullhtml="1" text=$smarty.capture.other_sorts|escape:"javascript"|escape:"html"} title="{tr}Other Sorts{/tr}">{icon name='list' alt="{tr}Other Sorts{/tr}"}</a></th>
			</tr>
			...
			{foreach from=$klist key=key item=item}
				{if $item->id ne ''}
					<tr{if ($key % 2)} class="odd"{else} class="even"{/if}>
						...
						<td class="text"><a href="#" title="{tr}Thumbnail{/tr}" {popup fullhtml="1" text=$smarty.capture.actions|escape:"javascript"|escape:"html"}><img class="athumb" src="{$item->thumbnailUrl}" alt="{$item->description}" height="80" width="120"></a></td>
						...
						<td class="text"><a href="#" title="{tr}Information{/tr}" {popup trigger="onmouseover" fullhtml="1" sticky=true text=$smarty.capture.add_info|escape:"javascript"|escape:"html" left=true}>{icon name='information' class='' title=''}</a></td>
					</tr>
				{/if}
			{/foreach}
		</table>
